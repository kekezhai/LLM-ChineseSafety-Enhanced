...
def gen(path, outpath):
    ...
    path = './Llama-3.2-1B-Instruct-ChineseSafety-Enhanced/checkpoint-9638'
    tokenizer = AutoTokenizer.from_pretrained(path, trust_remote_code=False)
    model = AutoModelForCausalLM.from_pretrained(path, trust_remote_code=False, torch_dtype=torch.float16, device_map=device)
    model = model.eval()
...
if __name__ == '__main__':
    ...
    model_name = 'Llama-3.2-1B-Instruct-ChineseSafety-Enhanced1.0'
    eva_set = 'zh' # 'zh' or 'en' or 'zh_subset'
